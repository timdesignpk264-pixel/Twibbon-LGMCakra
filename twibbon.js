...
const CSV_URL           = "Data formulir PK 264.csv";
...
// ---- helper: kirim info pilihan ke halaman induk (index-combined.html)
function notifyParentSelection(rec, displayName){
  try{
    const degree =
      String(rec?.["Jenis Gelar"] || rec?.["Jenjang"] || rec?.["Degree"] || "").trim();
    window.parent?.postMessage({
      type: "twibbon:selected",
      displayName: displayName || rec?.["Full Name"] || "",
      degree,
      record: rec || null
    }, "*");
  }catch(e){ console.warn("notifyParentSelection failed:", e); }
}
...
(async function init(){
 ...
 rowNameSelect?.addEventListener("change", async ()=>{
  ...
  draw();
  // beritahu parent setiap kali nama tampilan berubah
  notifyParentSelection(selected, currentDisplayName);
